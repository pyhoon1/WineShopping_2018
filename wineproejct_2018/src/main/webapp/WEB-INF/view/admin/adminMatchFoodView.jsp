				...
				if(document.getElementById('NoHave').innerHTML == '상품 중복 검사를 해주세요'){
					...
					$.ajax({
						...
						success : function(result) {
							if (result == "Y") {
								alert("상품 수정에 성공했습니다.");
								location.href="adminmatchFoodView.do?matchFoodId="+$('#matchfoodId').val();
							} else {
								alert("상품 수정에 실패했습니다.")
								return false;
							}
...
<body>
<input type="hidden" id="compareName" value="${matchFood.matchFoodName}">
...
<table>
			...
			<tr>
				<td>상품명</td>	
				<td><input type="text" id="matchFoodName" name="matchFoodName" value="${matchFood.matchFoodName}">
			...
			<tr>
				<td>가격</td>
				<td><input type="number" id="price" name="price" value="${matchFood.price}"></td>
			</tr>
			<tr>
				<td>원산지</td>
			...
			<tr>
				<td>중량</td>
				<td><input type="text" name="weight" id="weight" value="${matchFood.weight}"></td>
			</tr>
			<tr>
				<td>음식 설명</td>
